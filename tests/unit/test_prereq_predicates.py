...
def test_min_ram_mb_fires_when_below() -> None:
    rule = _rule(name="iosxr-ram", predicate_kind="min_ram_mb", predicate_args={"min_mb": 4096})
...
def test_min_ram_mb_silent_when_meets() -> None:
    rule = _rule(name="iosxr-ram", predicate_kind="min_ram_mb", predicate_args={"min_mb": 1024})
    dev = _device(ram_mb=2048)
...
def test_min_disk_mb_fires_when_below() -> None:
    rule = _rule(name="d", predicate_kind="min_disk_mb", predicate_args={"min_mb": 16384})
...
def test_min_disk_mb_silent_when_meets() -> None:
    rule = _rule(name="d", predicate_kind="min_disk_mb", predicate_args={"min_mb": 1024})
...
def test_min_current_version_fires_when_below() -> None:
    rule = _rule(
        name="mv", predicate_kind="min_current_version", predicate_args={"min_version": "7.6.0"}
    )
    b = prereq_predicates.eval_min_current_version(rule, _device(), _obs(observed_firmware="7.5.2"))
    assert b is not None
...
def test_min_current_version_silent_when_meets() -> None:
    rule = _rule(
        name="mv", predicate_kind="min_current_version", predicate_args={"min_version": "7.0.0"}
    )
    b = prereq_predicates.eval_min_current_version(rule, _device(), _obs(observed_firmware="7.5.2"))
    assert b is None
...
def test_min_current_version_missing_observation_field() -> None:
    ...
    b = prereq_predicates.eval_min_current_version(rule, _device(), _obs(observed_firmware=None))
    assert b is not None
    assert b.predicate_kind == "missing_observation_field"
...
def test_hardware_revision_in_fires_when_not_in_set() -> None:
    rule = _rule(
        name="hw",
        predicate_kind="hardware_revision_in",
...
def test_license_present_fires_when_missing() -> None:
    rule = _rule(
        name="lic", predicate_kind="license_present", predicate_args={"license": "advantage"}
    )
...
def test_license_present_silent_when_present() -> None:
    rule = _rule(name="lic", predicate_kind="license_present", predicate_args={"license": "base"})
    b = prereq_predicates.eval_license_present(rule, _device(licenses=["base"]), _obs())
    assert b is None
...
def test_intermediate_version_required_emits_recommended() -> None:
    rule = _rule(
        name="iv",
        predicate_kind="intermediate_version_required",
...
def test_not_in_state_silent_when_not_in_forbidden() -> None:
    rule = _rule(
        name="nis", predicate_kind="not_in_state", predicate_args={"states": ["uplift_planned"]}
    )
...
def test_region_in_fires_when_not_in_set() -> None:
    rule = _rule(
        name="ri", predicate_kind="region_in", predicate_args={"regions": ["bergen", "trondheim"]}
    )
...
def test_region_in_silent_when_in_set() -> None:
    rule = _rule(
        name="ri", predicate_kind="region_in", predicate_args={"regions": ["oslo", "bergen"]}
    )
...
def test_evaluate_rule_dispatches_to_correct_predicate() -> None:
    rule = _rule(name="r", predicate_kind="min_ram_mb", predicate_args={"min_mb": 4096})
...
def test_evaluate_rule_unknown_kind_raises() -> None:
    rule = _rule(name="r", predicate_kind="bogus_predicate", predicate_args={})
    import pytest
