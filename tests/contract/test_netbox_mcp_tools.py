"""Contract tests for F7 MCP tool definitions."""
...
def test_contract_yaml_exists_and_parses() -> None:
    assert CONTRACT.exists(), f"missing F7 MCP contract: {CONTRACT}"
    tools = _load_tools()
    assert tools, "contract declares zero tools"
    assert len(tools) == 1, f"F7 declares 1 MCP tool, got {len(tools)}"
