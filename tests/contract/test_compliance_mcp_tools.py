"""Contract test for F3 MCP tool definitions.

Parses ``specs/003-compliance-drift-evaluation/contracts/mcp-tools.yaml``
and asserts:

- Every tool declares ``name``, ``description``, ``auth``, ``input_schema``,
  ``output_schema``.
- ``auth`` is a value the Permission catalogue exposes.
...
that when F008 lands, every contracted tool already has a working
delegate.
"""
...
def test_contract_yaml_exists_and_parses() -> None:
    assert CONTRACT.exists(), f"missing F3 MCP contract: {CONTRACT}"
    tools = _load_tools()
    assert tools, "contract declares zero tools"
    assert len(tools) == 4, f"F3 declares 4 MCP tools, got {len(tools)}"
...
def test_auth_is_a_known_permission_attribute(tool: dict) -> None:
    """`auth` is the *attribute name* on `Permission` (e.g. READ_COMPLIANCE),
    not the permission string value. The MCP server resolves the
    attribute at registration time, so we check both halves here.
    """
    name = tool["auth"]
